...
// 어느 한 쪽의 객체를 변경하면 서로 영향 받는다.
console.log(person); // {name: "Kim", address: "Seoul"}
console.log(copy);   // {name: "Kim", address: "Seoul"}

```

- 원본(person)과 사본(copy)는 저장된 **메모리 주소는 다르지만 동일한 참조 값**을 갖는다.

- 이는, 원본과 사본이 모두 동일한 객체를 가리킨다는 의미이며, **"두개의 식별자가 하나의 객체를 공유"** 하는 것이다.
<br>

### 얕은 복사와 깊은 복사

> `객체를 프로퍼티 값으로 갖는 객체`의 경우 `얕은 복사와 깊은 복사로 나뉜다.`
<br>
...
- **깊은 복사(deep copy)** : 객체에 중첩되어 있는 객체까지 모두 복사하는 것
   ...
   - **공통점** : 얕은 복사(=원본) 와 깊은 복사(=복사본) 로 생성된 객체는 원본과는 다른(=참조값이 다른) 객체.
   - **차이점** : 얕은 복사는 참조 값을 복사하고, 깊은 복사는 객체까지 모두 복사해서 완전한 복사본을 만든다.
<br>
...
식별자가 기억하는 메모리 공간에 저장되어 있는 "값을 복사해서 잔달한다는 면이 동일"

그러나, 변수에 저장되어 있는 값이 "원시 값이냐 참조 값이냐의 차이만 있을 뿐"이다.
